...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY') or 'change-this-to-a-secure-random-value'
...
def index():
    ...
    return render_template('list.html', products=products, delete_form=delete_form)
...
def handle_bad_request(e):
    message = getattr(e, 'description', '') or str(e)
    if 'csrf' in message.lower():
        flash('Chybí CSRF token. Obnovte stránku a zkuste to znovu.', 'error')
        return redirect(url_for('index'))
    return e
